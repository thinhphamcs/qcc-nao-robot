...
var jsnao = {
  ...
  session : null,
  al_sys : null,
  al_dialog : null, // New Variable
  al_behavior : null, // New Variable
  al_tts : null,
  al_video : null,
  al_motion : null,
  al_posture : null,
  ...
  connected : function() {
    ...
    // New Service which calls new function
    jsnao.session.service("ALDialog").done(jsnao.init_dialog);
    jsnao.session.service("ALBehaviorManager").done(jsnao.init_behavior);
  },

  // This function is for forceInput
  init_dialog : function(data) { // Begin
     // Initializing an object
     jsnao.al_dialog = data;
     // This will forces the robot to take human input as inputs not outputs
     jsnao.al_dialog.forceInput();
     //This will let me see if the function is actually working or not
     console.log('Thinh Initialized');
  }, // End

  // This function is for loading topics
  init_behavior : function(data) { // Begin
      // Initializing an object
      jsnao.al_behavior = data;
      // This will make it as a block
      $('#LoadTopics').show(300);
      // This will make the button stay pressed
      $("#Directions").click(function() {
        if($(this).toggleClass("active")) {
            jsnao.al_behavior.startBehavior('dialog_school_direction-1a300c/behavior_1');
            jsnao.al_behavior.runBehavior('dialog_school_direction-1a300c/behavior_1');
          }
            jsnao.al_behavior.stopBehavior('dialog_school_direction-1a300c/behavior_1');
      });
      $("#Courses").click(function() {
        if($(this).toggleClass("active")) {
          jsnao.al_behavior.startBehavior('projectclass-809af0/behavior_1');
          jsnao.al_behavior.runBehavior('projectclass-809af0/behavior_1');
        }
          jsnao.al_behavior.stopBehavior('projectclass-809af0/behavior_1');
      });
  ...
  // This function will allow me to move the robot with user inputs
  init_motion : function(data) {
    // Initializing an object
    jsnao.al_motion = data;
    // This will take in the user inputs

    // This will allow the robot to move with user inputs
    // These are the turn left/right using Z
    $("#turn-left").click(function() {
      var z_left = $("#RbtSay").val();
      jsnao.al_motion.moveTo(0.0, 0.0, parseFloat(z_left)); // 0.26
    });
    $("#turn-right").click(function() {
      var z_right = $("#RbtSay").val();
      jsnao.al_motion.moveTo(0.0, 0.0, parseFloat(z_right)); // -0.26
    });
    // These are the go forward/backward using X
    $("#go-forward").click(function() {
      // if the string does not contain "," which is equals to -1
      if ($("#RbtSay").val().indexOf(',') == -1) {
         var x_forward = $("#RbtSay").val();
         jsnao.al_motion.moveTo(parseFloat(x_forward), 0.0, 0.0); // 0.1
      } else {
        var input = $("#RbtSay").val().split(",");
        var x_forward = input[0].trim();
        var z_forward = input[1].trim();
        jsnao.al_motion.moveTo(parseFloat(x_forward), 0.0, parseFloat(z_forward));
      }
    });
    $("#go-backward").click(function() {
      // if the string does not contain "," which is equals to -1
      if ($("#RbtSay").val().indexOf(',') == -1) {
         var x_backward = $("#RbtSay").val();
         jsnao.al_motion.moveTo(parseFloat(x_backward), 0.0, 0.0); // 0.1
      } else {
        var input = $("#RbtSay").val().split(",");
        var x_backward = input[0].trim();
        var z_backward = input[1].trim();
        jsnao.al_motion.moveTo(parseFloat(x_backward), 0.0, parseFloat(z_backward));
      }
    });
    // Thse are the move left/right using Y
    $("#move-left").click(function() {
      var y_left = $("#RbtSay").val();
      jsnao.al_motion.moveTo(0.0, parseFloat(y_left), 0.0); // 0.1
    });
    $("#move-right").click(function() {
      var y_right = $("#RbtSay").val();
      jsnao.al_motion.moveTo(0.0, parseFloat(y_right), 0.0); // -0.1
    });
    // This will make it as a block
    $('#DivMotion').show(300);
    // This will let me see if the function is actually working or not
